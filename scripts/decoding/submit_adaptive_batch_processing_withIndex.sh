...
for MAX_SEQUENCES in "${SEQUENCE_NUMS[@]}"; do
    ...
    for SUBFOLDER in ${RESULT_DIR}/*/; do
        ...
        # 设置输出路径（包含序列数信息）
        OUTPUT_CSV="${SUBFOLDER}/${SAMPLE_NAME}_consensus_results_adaptive_100bp_seq${MAX_SEQUENCES}.csv"
...
# 运行自适应聚类版本分析
python step2_consensus_comparison_adaptive_withIndex.py \\
...
echo "查看结果文件: ls -la ${RESULT_DIR}/*/*_consensus_results_adaptive_100bp_seq*.csv"
echo "========================================="
...
echo "   ls -la ${RESULT_DIR}/*/*_consensus_results_adaptive_100bp_seq*.csv"
echo ""
echo "2. 对比同一样本不同序列数的结果："
echo "   ls -la ${RESULT_DIR}/TN3055-2/*_consensus_results_adaptive_100bp_seq*.csv"
echo ""
echo "3. 分析成功率差异："

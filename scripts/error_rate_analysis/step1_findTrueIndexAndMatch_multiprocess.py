...
def process_chunk(sequence_data_chunk, library, index_list, aligner, q_threshold, 
                  primer1, primer2, is_reverse, 
                  index_length, primer_pos, index_pos,
                  payload_threshold, unique_threshold,
                  chunk_id):
    ...
    for sequence_data in sequence_data_chunk:
        ...
        if start_pos2 - end_pos1 <= payload_threshold or score1 < 20 or score2 < 21:  # 引物位置不正确
            continue

        # 根据index寻找library中与read匹配的ideal_seq
        sub_read_seq = sequence[start_pos2-index_length:start_pos2]
        if len(sub_read_seq) < index_length:  # 索引长度不够
            continue
        ...
        if best_score == len(sub_read_seq):  # 能够找到完全匹配的索引
            ...
            sub_read_seq = sequence[end_pos1:start_pos2-index_length]
            alignment = aligner.align(sub_read_seq, most_similar_string[primer_pos[0]:index_pos[0]])
            best_score = alignment.score
            if best_score < payload_threshold:  # payload相似度不符合条件，找相似的 unique，获得对应的索引和序列
                seq_unique = sequence[end_pos1:start_pos2]
                for index, ideal_seq in enumerate(library):
                    alignment = aligner.align(seq_unique, ideal_seq[primer_pos[0]:primer_pos[1]])
                    score = alignment.score
                    if score > best_score:
                        best_score = score
                        most_similar_string = ideal_seq
        ...
        # 增加一个判定条件，舍弃相似度较差的序列
        if best_score < unique_threshold:
            continue
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        futures = [
            executor.submit(process_chunk, 
                            ...
                            primer1, primer2, is_reverse, 
                            args.index_length, args.primer_pos, args.index_pos,
                            args.payload_threshold, args.unique_threshold,
                            i)
